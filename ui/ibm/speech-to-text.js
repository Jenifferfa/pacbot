    ...
    if (recording === false) {
      if (records === 0) {
        ...
        Api.setWatsonPayload({
          output: { text: ['É necessário permitir o microfone.'], ref: 'STT' },
  ...
  function speechToText() {
    ...
    fetch('/api/speech-to-text/token') // Fetch authorization token for Watson Speech-To-Text
      ...
      .then(function(token) {
        ...
        stream
          .promise() // Once all data has been processed...
          .then(function(data) {
            // ...put all of it into a single array
            ...
            if (data.length !== 0) {
              // If data is not empty (the user said something)
              ...
              if (
                dialogue.alternatives[0].transcript !== '' &&
                dialogue.final === true
              ) {
                // Another check to verify that the transcript is not empty and that this is the final dialog
                Conversation.sendMessage(); // Send the message to Watson Conversation
              }
            } else {
              // If there isn't any data to be handled by the conversation, display a message to the user letting them know
              Api.setWatsonPayload({
                output: {
                  text: [
                    'Microphone input cancelled. Please press the button to speak to Watson again',
                  ],
                },
              }); // If the user clicked the microphone button again to cancel current input
            }
          })
          .catch(function(err) {
            // Catch any errors made during the promise
            if (err !== 'Error: No speech detected for 5s.') {
              // This error will always occur when Speech-To-Text times out, so don't log it (but log everything else)
              console.log(err);
            }
            mic.setAttribute('class', 'inactive-mic'); // Reset our microphone button to visually indicate we aren't listening to user anymore
            Api.setWatsonPayload({
