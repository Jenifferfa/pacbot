...
var Animations = (function() {
  ...
  function newDropline(x, y) {
    ...
    var dropPath =
      ...
      ',l,' +
      [68.031 * scale, -57.107 * scale] +
      ',l,' +
      [-57.107 * scale, 68.034 * scale] +
      ',c,' +
      [
        -5.725604 * scale,
        6.8212 * scale,
  ...
  function toggleRain() {
    ...
    function animateDrops() {
      ...
      lowerDrops.animate(
        {
          transform:
            't' + [Math.random() * 50, topTransform[1] + fallDistance / 2.0],
        },
        2500,
        mina.linear,
        function() {
          ...
          lowerDrops.animate(
            {
              transform:
                't' + [Math.random() * 50, topTransform[1] + fallDistance],
            },
            2500,
            mina.linear,
            function() {
  ...
  function toggleDarkenSky() {
    ...
    Common.listForEach(
      document.getElementsByClassName(classes.darkCloud),
      function(currentElement) {
        Common.toggleClass(currentElement, classes.lightGrayCloud);
      },
    );
    Common.listForEach(
      document.getElementsByClassName(classes.lightCloud),
      function(currentElement) {
    ...
    rightNeedle.animate(
      {
        transform:
          ...
          (45 * Math.random() - 30) +
          ', ' +
          speedometer.getBBox().cx +
          ',' +
          speedometer.getBBox().cy,
      },
      9000 * Math.random(),
      mina.easeinout,
      function() {
  ...
  function lightsOn() {
    ...
    Snap.select(idSelectors.headlights).animate(
      { opacity: 1 },
      300,
      mina.linear,
    );
  ...
  function wipersOn(speed) {
    ...
    if (state.wipingAnim) {
      // If animation is already going on return
      return;
    }
    moveWipers();
  }
  ...
  function rotateWipers(from, to, next) {
    ...
    state.wipingAnim = Snap.animate(
      from,
      to,
      function(val) {
        rWiper.transform(
          'r' +
            [val, rWiper.bbox.x + rWiper.bbox.w, rWiper.bbox.y + rWiper.bbox.h],
        );
        lWiper.transform(
          'r' +
            [val, lWiper.bbox.x + lWiper.bbox.w, lWiper.bbox.y + lWiper.bbox.h],
        );
      },
      2000 / Math.max(speeds[wiperSpeed], speeds.lo),
      mina.linear,
      next,
      frameSkipRate,
    );
